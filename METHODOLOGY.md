...
## 10a. When the printed report arrives before the data file

The Mayor's Management Report is published twice a year as a PDF, and its indicators are also published as a dataset (rbed-zzin) on NYC Open Data. The two do not land together. The fiscal 2026 report was released Sept. 17, 2026; the dataset was still carrying the preliminary report, whose last fiscal 2026 figures cover July 2025 through March 2026.

Where that happens, the registry takes the fiscal year's figure from the printed report and says so in the figure's period line, its source line and its caveats. Three rules govern it:

- **The row has to prove it is the right row.** `scripts/fetch_mmr_pdf.py` does not match an indicator by name. It reads every performance-indicator table in the report, then accepts a row for an indicator only if that row's own fiscal 2022 to 2025 columns equal the values the dataset publishes for those years. Where two rows both clear that bar, the one whose printed label matches the indicator name is used, and only if it is clearly closer than the runner-up; otherwise the script stops. It also stops if any selected indicator cannot be placed at all. For the 69 indicators read on Sept. 17, 2026, every printed fiscal 2022 to 2025 column matched the dataset, so the report restated nothing.
- **The quote is the table row.** The source quote stored with a printed-report figure is the row as the report prints it, label and all five years, with the page number. The mechanical check finds that row in the local copy of the report text and re-reads the fiscal 2026 column out of it.
- **The definition still comes from the dataset.** The printed report does not repeat the indicator definitions in its tables, so the definition shown with the figure is the dataset's own description field from the last year it published one. Where that matters it is in the caveats.

When the dataset catches up, the build switches back to it on its own: the printed figure is used only while Open Data has no June row for the fiscal year.
