...
## Update, Sept. 17, 2026: the fiscal 2026 Mayor's Management Report

The city released the fiscal 2026 report on Sept. 17, 2026; the copy read here carries a last-modified time of 4:42 p.m. ET that day and a sha256 recorded in data/mmr_fy2026_pdf.json. NYC Open Data's indicator file (rbed-zzin) had not been refreshed past the preliminary report, so the 69 selected indicators were read out of the printed report by `scripts/fetch_mmr_pdf.py` and 65 registry figures moved from a partial year to full fiscal 2026 (July 1, 2025 to June 30, 2026). Checks run on that update:

- **Row identification.** No indicator was matched by name. A printed row was accepted only when its own fiscal 2022 to 2025 columns equaled the values NYC Open Data publishes for those years; where two rows cleared that bar, the closer printed label had to win by a clear margin. All 69 were placed, and no printed column disagreed with the dataset, so the report restated no earlier year.
- **Mechanical re-check.** `scripts/blind_check.py` re-finds each quoted row in the local copy of the report text at the cited page, re-reads the fiscal 2026 column out of it and compares it to the registry, then re-queries Open Data live for the fiscal 2025 column the same row prints. 78 figures checked, no value, date, definition or quote mismatch; the three computed (A-c) figures are left to the blind pass, as before.
- **One indicator did not report.** HRA's Medicaid application timeliness rate prints NA for fiscal 2025 and fiscal 2026; that line still shows its fiscal 2024 figure and says so.
- **One rename found.** The printed report calls HRA's "Average wait time to speak with a customer service agent" an "Average customer in-person wait time" (page 234). The series is identical back to fiscal 2022, and the new name matches the definition the registry has used since Sept. 5. NYC Open Data still carries the old name. Recorded in data/source_issues.md.

- **Blind pass, fourth checker.** A checker barred from data/ and scripts/, and given only the indicator names and the report text, re-extracted all 69 fiscal 2026 actuals (data/blind_check_fy2026.json). It agreed on the value and the page for 68. The 69th is the HRA row above: the checker reported that no row called “Average wait time to speak with a customer service agent” exists in the HRA chapter, which is the rename, found independently. Every prior-year column it read matched as well.
